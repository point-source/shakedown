...
def _scenario_setup_readiness() -> ScenarioResult:
    with TemporaryDirectory() as tmp:
        ...
        rc = 0 if validate_config(config).ok else 1
...
def _scenario_handoff_failure() -> ScenarioResult:
    with TemporaryDirectory() as tmp:
        ...
        return ScenarioResult(
            "handoff failure",
            rc == 1
            and "RECOVERY: validate failed_before_completion" in status_text
            and "handoff" in status_text,
...
def _scenario_partial_failure_recovery() -> ScenarioResult:
    ...
    with TemporaryDirectory() as tmp:
        ...
        checks.append(
            failed == 1
            and "RECOVERY: sync completed_with_item_issues" in sync_failed_status
            and retried == 0
        ...
        checks.append(
            restage_failed == 1
            and "RECOVERY: restage completed_with_item_issues" in restage_failed_status
        ...
        checks.append(
            reconcile_failed == 1
            and "RECOVERY: reconcile stale_enumeration" in reconcile_failed_status
            and reconcile_retried == 0
            and "RECOVERY:" not in reconcile_recovered_status
        )
        details.append("reconcile")

        validation_config = _config(root / "validate", "validate-recovery")
        _ReleaseFakePlugin.fail_enumerate = True
        validate_failed = 0 if validate_config(validation_config).ok else 1
        validate_failed_status = _status_text(validation_config)
        _ReleaseFakePlugin.fail_enumerate = False
        validate_retried = 0 if validate_config(validation_config).ok else 1
        validate_recovered_status = _status_text(validation_config)
        checks.append(
            validate_failed == 1
            and "RECOVERY: validate failed_before_completion" in validate_failed_status
